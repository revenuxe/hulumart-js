...
const getCatalog = unstable_cache(
  async (): Promise<Catalog> => {
    ...
    if (categoriesError || subcategoriesError || productsError)
      throw categoriesError ?? subcategoriesError ?? productsError;
    ...
    const categories: CatalogCategory[] = (categoryRows ?? []).map((row) => ({
      ...
      name: row.name,
      tagline: row.tagline ?? "",
      accent: row.accent ?? "from-slate-900/80 to-indigo-700/70",
      heroImage: row.image_url ?? "",
      sortOrder: row.sort_order,
      updatedAt: row.updated_at,
    }));
    const subcategories: CatalogSubcategory[] = (subcategoryRows ?? []).map(
      (row) => ({
        id: row.id,
        slug: row.slug,
    ...
    const services: CatalogProduct[] = (productRows ?? []).map((row) => {
      ...
      return {
        ...
        approximateAgeMonths: row.approximate_age_months ?? undefined,
        usageSummary: row.usage_summary ?? undefined,
        warrantyStatus: row.warranty_status,
        warrantyProvider: row.warranty_provider ?? undefined,
        warrantyExpiresAt: row.warranty_expires_at ?? undefined,
        warrantyCoverage: row.warranty_coverage ?? undefined,
        warrantyTransferable: row.warranty_transferable,
        ...
        priceDiscounted,
        discountPct:
          row.sale_price != null && row.price > 0
        ...
        faqs: ((row.faqs ?? []) as ProductFaq[]).filter(
          (faq) =>
...
export async function getSubcategoriesByCategory(categorySlug: string) {
  return (await getCatalog()).subcategories.filter(
    (item) => item.categorySlug === categorySlug,
  );
}
export async function getAllSubcategories() {
  return (await getCatalog()).subcategories;
}
...
export async function getSubcategoryBySlug(
  ...
  return (await getCatalog()).subcategories.find(
    (item) =>
...
export async function getServiceBySlug(
  ...
  return (await getCatalog()).services.find(
    (item) => item.categorySlug === categorySlug && item.slug === serviceSlug,
  );
}
export async function getTrendingServices(limit = 8) {
  return (await getCatalog()).services
...
export const getHomepageHeroSlides = unstable_cache(
  async (): Promise<HomepageHeroSlide[]> => {
    const { data, error } = await publicSupabaseClient()
    ...
    return (data ?? []).map((slide) => ({
      ...
      actionLabel: slide.action_label,
      actionUrl: getHomepageHeroActionUrl(slide.action_url),
    }));
  },
  ["homepage-hero"],
