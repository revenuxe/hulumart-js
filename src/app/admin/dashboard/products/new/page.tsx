...
async function fetchProductFormOptions() {
  ...
  const [{ data: cats, error: categoriesError }, { data: subs, error: subcategoriesError }, { data: products, error: productsError }, { data: addons, error: addonsError }] = await Promise.all([
    supabase.from("categories").select("id,name").order("name"),
    supabase.from("subcategories").select("id,name,category_id").order("name"),
    supabase.from("products").select("tags"),
    supabase.from("addons").select("id,name,price").order("sort_order"),
  ]);
  if (categoriesError || subcategoriesError || productsError || addonsError) {
    throw categoriesError ?? subcategoriesError ?? productsError ?? addonsError;
  }
